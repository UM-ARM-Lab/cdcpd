...
int main(int argc, char *argv[])
{
  ...
  // service to reset tracking
  boost::function<bool(std_srvs::EmptyRequest &, std_srvs::EmptyResponse &)> reset_cb =
  ...
  auto const reset_service = ph.advertiseService("reset", reset_cb);
  ...
  auto const object_template = makeRopeTemplate(num_points, length);
  auto const template_vertices = object_template.first;
  auto const template_edges = object_template.second;
  ...
  auto cdcpd = std::make_unique<CDCPD>(template_cloud, template_edges, use_recovery, alpha, beta, lambda, k_spring);
  ...
  auto const callback = [&](cv::Mat rgb, cv::Mat depth, cv::Matx33d cam)
  {
    ...
    auto out = cdcpd->operator()(rgb, depth, hsv_mask, cam, template_cloud, true, false, fixed_points);
