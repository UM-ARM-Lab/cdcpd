...
std::tuple <Eigen::Matrix3Xf, Eigen::Matrix2Xi>
init_template(const bool is_rope, const Matrix3Xf &init_points, int points_on_rope, int num_width, int num_height)
{
...
int main(int argc, char *argv[])
{
  ...
  auto sync_use_gripper = message_filters::Synchronizer<SyncPolicy_use_gripper>(SyncPolicy_use_gripper(10),
                                                                                rgb_sub_use_gripper,
                                                                                depth_sub_use_gripper,
                                                                                info_sub_use_gripper, config_sub,
                                                                                dot_sub, l_sub, r_sub);
...
out_without_constrain.gurobi_output->header.frame_id = frame_id;
    ...
    if (is_no_pred)
    {
      ...
      if (is_sim)
      {
        out_without_prediction = cdcpd_without_prediction(rgb_image, depth_image, hsv_mask, intrinsics,
                                                          template_cloud_without_prediction, g_dot, g_config, false,
                                                          false, false, 0, fixed_points);
      ...
      } else
      {
        ...
        {
          out_without_prediction = cdcpd_without_prediction(rgb_image, depth_image, hsv_mask, intrinsics,
                                                            template_cloud_without_prediction, g_dot, g_config,
                                                            is_grasped, nh_ptr, translation_dir_deformability,
                                                            translation_dis_deformability, rotation_deformability,
                                                            false, false, false, 0, fixed_points);
        } else
        {
          out_without_prediction = cdcpd_without_prediction(rgb_image, depth_image, hsv_mask, intrinsics,
                                                            template_cloud_without_prediction, false, false, false, 0,
                                                            fixed_points);
        }
      }
    ...
    if (is_no_pred)
    {
      ...
      if (!is_rope)
      {
        order_without_pred = pc_to_marker(out_without_prediction.gurobi_output, template_edges, frame_id, num_height,
                                          num_width);
      } else
      {
        order_without_pred = pc_to_marker(out_without_prediction.gurobi_output, template_edges, frame_id, 1,
                                          points_on_rope);
      }
