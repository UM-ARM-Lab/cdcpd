...
typedef message_filters::sync_policies::ApproximateTime<
    sm::Image, sm::Image, sm::CameraInfo, cdcpd_ros::Float32MultiArrayStamped, cdcpd_ros::Float32MultiArrayStamped,
    victor_hardware_interface::Robotiq3FingerStatus_sync, victor_hardware_interface::Robotiq3FingerStatus_sync>
    SyncPolicy_use_gripper;

typedef message_filters::sync_policies::ApproximateTime<sm::Image, sm::Image, sm::CameraInfo> SyncPolicy_img;
...
cv::Mat draw_vis(const cv::Mat &rgb_image, const Matrix3Xf &template_vertices, const Matrix2Xi &template_edges,
                 const Matx33d &intrinsics, const cv::Scalar &color) {
...
vm::Marker draw_cylinder_marker(const std::vector<float> &cylinder_data, const std::vector<float> &quat,
...
static victor_hardware_interface::Robotiq3FingerStatus_sync::ConstPtr gripper_status_origin_to_sync(
  ...
  victor_hardware_interface::Robotiq3FingerStatus_sync::ConstPtr syncptr(
      new victor_hardware_interface::Robotiq3FingerStatus_sync(sync));
...
void callback_use_gripper(const sm::Image::ConstPtr &rgb_img, const sm::Image::ConstPtr &depth_img,
                          const sm::CameraInfo::ConstPtr &cam_info,
                          const cdcpd_ros::Float32MultiArrayStamped::ConstPtr &g_config,
                          const cdcpd_ros::Float32MultiArrayStamped::ConstPtr &g_dot,
                          const victor_hardware_interface::Robotiq3FingerStatus_sync::ConstPtr &l_s,
...
void callback_im(const sm::Image::ConstPtr &rgb_img, const sm::Image::ConstPtr &depth_img,
                 const sm::CameraInfo::ConstPtr &cam_info) {
  color_images.push_back(rgb_img);
  depth_images.push_back(depth_img);
  camera_infos.push_back(cam_info);
...
std::tuple<cv::Mat, cv::Mat, cv::Matx33d> toOpenCv(const sm::Image::ConstPtr &rgb_img,
                                                   const sm::Image::ConstPtr &depth_img,
                                                   const sm::CameraInfo::ConstPtr &cam_info, bool is_sim) {
...
std::vector<bool> toGripperStatus(const victor_hardware_interface::Robotiq3FingerStatus_sync::ConstPtr &l_ptr,
...
std::tuple<AllGrippersSinglePose, AllGrippersSinglePoseDelta> toGripperConfig(
    const cdcpd_ros::Float32MultiArrayStamped::ConstPtr &g_config,
    const cdcpd_ros::Float32MultiArrayStamped::ConstPtr &g_dot) {
...
std::tuple<Eigen::Matrix3Xf, Eigen::Matrix2Xi> init_template(const bool is_rope, const Matrix3Xf &init_points,
                                                             int points_on_rope, int num_width, int num_height) {
  ...
  if (is_rope) {
    ...
    // float left_x = -0.5f; float left_y = -0.5f; float left_z = 3.0f; float right_x = 0.5f; float right_y = -0.5f;
    // float right_z = 3.0f;

    // rope_edge_cover_1
    // float left_x = -0.5f; float left_y = -0.0f; float left_z = 1.0f; float right_x = 0.3f; float right_y = -0.0f;
    // float right_z = 1.0f;

    // rope_winding_cylinder_exp_1_comp
    // float left_x = -0.5f; float left_y = -0.2f; float left_z = 1.0f; float right_x = 0.3f; float right_y = -0.2f;
    // float right_z = 1.0f;

    // rope_winding_cylinder_exp_2_comp
    // float left_x = -0.5f; float left_y = 0.2f; float left_z = 1.0f; float right_x = 0.3f; float right_y = 0.2f; float
    // right_z = 1.0f;

    // rope_by_hand_3
    // float left_x = -0.5f; float left_y = 0.2f; float left_z = 2.0f; float right_x = 0.3f; float right_y = 0.2f; float
    // right_z = 2.0f;
    ...
    // int num_height = 15;
    // This is for simulation
    // int num_width = 20; int num_height = 20; float right_up_y = 0.19f; float right_up_x = 0.19f; float left_bottom_y
    // = -0.19f; float left_bottom_x = -0.19f; float z = 2.0f; int num_width = 15; int num_height = 15; float right_up_y
    // = 0.14f; float right_up_x = 0.14f; float left_bottom_y = -0.14f; float left_bottom_x = -0.14f; float z = 1.0f;
    // cloth_cover_by_hand
    // int num_width = 15; int num_height = 15; float right_up_y = 0.0f; float right_up_x = -0.1f; float left_bottom_y =
    // -0.28f; float left_bottom_x = -0.38f; float z = 1.5f; some data on 10/10 int num_width = 15; int num_height = 15;
    // float rb_y = 0.13f; float rb_x = -0.3f; float rb_z = 1.2f; float lb_y = 0.13f; float lb_x = -0.58f; float lb_z
    // = 1.2f; float lt_y = 0.27f; float lt_x = -0.58f; float lt_z = 0.96f;
...
std::tuple<Eigen::Matrix3Xf, Eigen::Matrix2Xi> init_template() {
#ifdef ROPE
  // This is for simulation
  // float left_x = -0.5f; float left_y = -0.5f; float left_z = 3.0f; float right_x = 0.5f; float right_y = -0.5f; float
  // right_z = 3.0f;

  // rope_edge_cover_1
  // float left_x = -0.5f; float left_y = -0.0f; float left_z = 1.0f; float right_x = 0.3f; float right_y = -0.0f; float
  // right_z = 1.0f;
  ...
  int num_width = 15;
  int num_height = 15;
  float right_up_z = 1.28f;
  float right_up_x = -0.1f;
  float left_bottom_z = 1.00f;
  float left_bottom_x = -0.38f;
  float y = 0.0f;
...
void test_velocity_calc() {
  ...
  g_current(0, 0) = -0.28453702;
  g_current(0, 1) = -0.95784968;
  g_current(0, 2) = 0.03952325;
  g_current(0, 3) = 0.49164355;
  g_current(1, 0) = 0.27036124;
  g_current(1, 1) = -0.04062294;
  g_current(1, 2) = 0.96190161;
  g_current(1, 3) = -0.49191713;
  g_current(2, 0) = -0.91975188;
  g_current(2, 1) = 0.28438324;
  g_current(2, 2) = 0.27052405;
  g_current(2, 3) = 2.973032;
  g_current(3, 0) = 0.;
  g_current(3, 1) = 0.;
  g_current(3, 2) = 0.;
  g_current(3, 3) = 1.;

  g_next(0, 0) = -0.44558436;
  g_next(0, 1) = -0.88946199;
  g_next(0, 2) = 0.10154484;
  g_next(0, 3) = 0.47975183;
  g_next(1, 0) = 0.4167676;
  g_next(1, 1) = -0.1057118;
  g_next(1, 2) = 0.90284544;
  g_next(1, 3) = -0.4806639;
  g_next(2, 0) = -0.79231262;
  g_next(2, 1) = 0.44461477;
  g_next(2, 2) = 0.41780227;
  g_next(2, 3) = 2.94883704;
  g_next(3, 0) = 0.;
  g_next(3, 1) = 0.;
  g_next(3, 2) = 0.;
  g_next(3, 3) = 1.;
...
int main(int argc, char *argv[]) {
  ...
  // message_filters::Subscriber<cdcpd_ros::Float32MultiArrayStamped> config_sub(nh,
  // "/kinect2_victor_head/qhd/gripper_config", 10); message_filters::Subscriber<cdcpd_ros::Float32MultiArrayStamped>
  // dot_sub(nh, "/kinect2_victor_head/qhd/dot_config", 10);
  ...
  auto sync_use_gripper = message_filters::Synchronizer<SyncPolicy_use_gripper>(
      SyncPolicy_use_gripper(10), rgb_sub_use_gripper, depth_sub_use_gripper, info_sub_use_gripper, config_sub, dot_sub,
      l_sub, r_sub);
  ...
  if (republish_bagfile) {
    ...
    while (ros::ok() && color_iter != color_images.cend() && depth_iter != depth_images.cend() &&
  ...
  if (is_sim) {
    ...
    cdcpd = CDCPD(template_cloud, template_edges, nh_ptr, translation_dir_deformability, translation_dis_deformability,
                  rotation_deformability, g_ind,
#ifdef SHAPE_COMP
                  obstacle_param,
#endif
                  false, alpha, beta, lambda, k_spring, zeta, cylinder_data, is_sim);
  } else {
    cdcpd = CDCPD(template_cloud, template_edges,
#ifdef SHAPE_COMP
                  obstacle_param,
#endif
                  false, alpha, beta, lambda, k_spring, zeta, cylinder_data, is_sim);
  ...
  CDCPD cdcpd_without_constrain(template_cloud_without_constrain, template_edges, intrinsics, false, alpha, beta,
                                lambda, k_spring);
#endif
  ...
  if (is_no_pred) {
    if (is_sim) {
      cdcpd_without_prediction =
          CDCPD(template_cloud_without_prediction, template_edges, nh_ptr, translation_dir_deformability,
                translation_dis_deformability, rotation_deformability, g_ind,
#ifdef SHAPE_COMP
                obstacle_param,
#endif
                false, alpha, beta, lambda, k_spring, zeta, cylinder_data, is_sim);
    } else {
      cdcpd_without_prediction = CDCPD(template_cloud_without_prediction, template_edges,
#ifdef SHAPE_COMP
                                       obstacle_param,
#endif
                                       false, alpha, beta, lambda, k_spring, zeta, cylinder_data, is_sim);
  ...
  if (is_pred1) {
    if (is_sim) {
      cdcpd_pred1 = CDCPD(template_cloud_pred1, template_edges, nh_ptr, translation_dir_deformability,
                          translation_dis_deformability, rotation_deformability, g_ind,
#ifdef SHAPE_COMP
                          obstacle_param,
#endif
      ...
      cdcpd_pred1 = CDCPD(template_cloud_pred1, template_edges,
#ifdef SHAPE_COMP
                          obstacle_param,
#endif
                          false, alpha, beta, lambda, k_spring, zeta, cylinder_data, is_sim);
  ...
  if (is_pred2) {
    if (is_sim) {
      cdcpd_pred2 = CDCPD(template_cloud_pred2, template_edges, nh_ptr, translation_dir_deformability,
                          translation_dis_deformability, rotation_deformability, g_ind,
#ifdef SHAPE_COMP
                          obstacle_param,
#endif
      ...
      cdcpd_pred2 = CDCPD(template_cloud_pred2, template_edges,
#ifdef SHAPE_COMP
                          obstacle_param,
#endif
                          false, alpha, beta, lambda, k_spring, zeta, cylinder_data, is_sim);
  ...
  if (use_grippers) {
    ...
    Eigen::Vector3f left_pos((float)leftTS.transform.translation.x, (float)leftTS.transform.translation.y,
                             (float)leftTS.transform.translation.z);
    Eigen::Vector3f right_pos((float)rightTS.transform.translation.x, (float)rightTS.transform.translation.y,
                              (float)rightTS.transform.translation.z);
  ...
  string time_str = std::to_string(1 + ltm->tm_mon) + "_" + std::to_string(ltm->tm_mday) + "_" +
  ...
  if (is_record) {
    ...
    video_pred0.open(workingDir + "/../video/ICRA/" + bagfile + "_" + time_str + "/pred_0.avi",
                     CV_FOURCC('M', 'J', 'P', 'G'), fps, cv::Size(color_image_init.cols, color_image_init.rows));
    if (is_pred1) {
      video_pred1.open(workingDir + "/../video/ICRA/" + bagfile + "_" + time_str + "/pred_1.avi",
                       CV_FOURCC('M', 'J', 'P', 'G'), fps, cv::Size(color_image_init.cols, color_image_init.rows));
    }
    if (is_pred2) {
      video_pred2.open(workingDir + "/../video/ICRA/" + bagfile + "_" + time_str + "/pred_2.avi",
                       CV_FOURCC('M', 'J', 'P', 'G'), fps, cv::Size(color_image_init.cols, color_image_init.rows));
    }
    if (is_no_pred) {
      video_cdcpd.open(workingDir + "/../video/ICRA/" + bagfile + "_" + time_str + "/cdcpd.avi",
                       CV_FOURCC('M', 'J', 'P', 'G'), fps, cv::Size(color_image_init.cols, color_image_init.rows));
    }
    if (is_cpd_physics) {
      video_cpd_phy.open(workingDir + "/../video/ICRA/" + bagfile + "_" + time_str + "/cpd_physics.avi",
                         CV_FOURCC('M', 'J', 'P', 'G'), fps, cv::Size(color_image_init.cols, color_image_init.rows));
    }
    // if (is_combined_truth) {
    // 	video_comb.open(workingDir+"/../video/ICRA/" + bagfile + "_" + time_str +
    // "/combined.avi",CV_FOURCC('M','J','P','G'), 10, cv::Size(color_image_init.cols,color_image_init.rows));
    // }
    if (is_truth) {
      video_truth.open(workingDir + "/../video/ICRA/" + bagfile + "_" + time_str + "/truth.avi",
                       CV_FOURCC('M', 'J', 'P', 'G'), fps, cv::Size(color_image_init.cols, color_image_init.rows));
    }
  }
  while (color_iter != color_images.cend() && depth_iter != depth_images.cend() && info_iter != camera_infos.cend()) {
    ...
    if (is_sim) {
      out = cdcpd(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud, g_dot, g_config, true, is_interaction,
                  true, 0, fixed_points);
      std::ofstream(workingDir + "/error.txt", std::ofstream::app)
          << calc_mean_error(out.gurobi_output->getMatrixXfMap(), one_frame_truth) << " ";
    } else if (is_gripper_info) {
      ...
      out = cdcpd(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud, g_dot, g_config, is_grasped, nh_ptr,
                  translation_dir_deformability, translation_dis_deformability, rotation_deformability, true,
                  is_interaction, true, 0, fixed_points);
    } else {
      out = cdcpd(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud, true, is_interaction, true, 0,
                  fixed_points);
    }
...
#ifdef COMP
    auto out_without_constrain = cdcpd_without_constrain(
        rgb_image, depth_image, hsv_mask, template_cloud_without_constrain, template_edges, false, false);
    ...
    if (is_no_pred) {
      ...
      if (is_sim) {
        out_without_prediction =
            cdcpd_without_prediction(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud_without_prediction,
                                     g_dot, g_config, false, false, false, 0, fixed_points);
        std::ofstream(workingDir + "/error_no_pred.txt", std::ofstream::app)
            << calc_mean_error(out_without_prediction.gurobi_output->getMatrixXfMap(), one_frame_truth) << " ";
      } else {
        if (is_gripper_info) {
          out_without_prediction = cdcpd_without_prediction(
              rgb_image, depth_image, hsv_mask, intrinsics, template_cloud_without_prediction, g_dot, g_config,
              is_grasped, nh_ptr, translation_dir_deformability, translation_dis_deformability, rotation_deformability,
              false, false, false, 0, fixed_points);
        } else {
          out_without_prediction =
              cdcpd_without_prediction(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud_without_prediction,
                                       false, false, false, 0, fixed_points);
    ...
    if (is_pred1) {
      ...
      if (is_sim) {
        out_pred1 = cdcpd_pred1(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud_pred1, g_dot, g_config,
                                true, is_interaction, true, 1, fixed_points);
        std::ofstream(workingDir + "/error_pred1.txt", std::ofstream::app)
            << calc_mean_error(out_pred1.gurobi_output->getMatrixXfMap(), one_frame_truth) << " ";
      } else {
        out_pred1 = cdcpd_pred1(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud_pred1, g_dot, g_config,
                                is_grasped, nh_ptr, translation_dir_deformability, translation_dis_deformability,
                                rotation_deformability, true, is_interaction, true, 1, fixed_points);
      }
    ...
    if (is_pred2) {
      ...
      if (is_sim) {
        out_pred2 = cdcpd_pred2(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud_pred2, g_dot, g_config,
                                true, is_interaction, true, 2, fixed_points);
        std::ofstream(workingDir + "/error_pred2.txt", std::ofstream::app)
            << calc_mean_error(out_pred2.gurobi_output->getMatrixXfMap(), one_frame_truth) << " ";
      } else {
        out_pred2 = cdcpd_pred2(rgb_image, depth_image, hsv_mask, intrinsics, template_cloud_pred2, g_dot, g_config,
                                is_grasped, nh_ptr, translation_dir_deformability, translation_dis_deformability,
                                rotation_deformability, true, is_interaction, true, 2, fixed_points);
      }
    ...
    cpd_phy_result.open(
        ros::package::getPath("cdcpd_ros") + "/src/cpd_physics/" + bagfile + "/result" + std::to_string(frame) + ".txt",
        ios::in);
    // cout << "try to open cpd physics file " << ros::package::getPath("cdcpd_ros") + "/src/cpd_physics/" + bagfile +
    // "/result" + std::to_string(frame) + ".txt" << endl;
    if (cpd_phy_result.is_open()) {
      ...
      if (is_sim) {
        std::ofstream(workingDir + "/error_cpd_physics.txt", std::ofstream::app)
            << calc_mean_error(cpd_phy_pc->getMatrixXfMap().topRows(3).rowwise().reverse(), one_frame_truth) << " ";
      }
      ...
      cv::Mat demo_cpd_phy = draw_vis(draw_img, cpd_phy_pc->getMatrixXfMap().topRows(3), cpd_phy_edges, intrinsics,
                                      cv::Scalar(0, 255, 255));
      if (is_combined_truth) {
        demo_cpd_phy = draw_vis(demo_cpd_phy, one_frame_truth, template_edges, intrinsics, cv::Scalar(0, 0, 255));
      }
    ...
    if (is_no_pred) {
      ...
      if (!is_rope) {
        order_without_pred =
            pc_to_marker(out_without_prediction.gurobi_output, template_edges, frame_id, num_height, num_width);
      } else {
        order_without_pred =
            pc_to_marker(out_without_prediction.gurobi_output, template_edges, frame_id, 1, points_on_rope);
      }
    ...
    if (is_record) {
      ...
      cv::Mat demo_im = draw_vis(draw_img, out.gurobi_output->getMatrixXfMap().topRows(3), template_edges, intrinsics,
                                 cv::Scalar(0, 255, 255));
      if (is_combined_truth) {
        demo_im = draw_vis(demo_im, one_frame_truth, template_edges, intrinsics, cv::Scalar(0, 0, 255));
      }
      ...
      if (is_pred1) {
        ...
        cv::Mat demo_pred1 = draw_vis(draw_img, out_pred1.gurobi_output->getMatrixXfMap().topRows(3), template_edges,
                                      intrinsics, cv::Scalar(0, 255, 255));
        if (is_combined_truth) {
          demo_pred1 = draw_vis(demo_pred1, one_frame_truth, template_edges, intrinsics, cv::Scalar(0, 0, 255));
        }
        video_pred1.write(demo_pred1);
      }
      if (is_pred2) {
        // cv::Mat demo_pred2 = padding_img(draw_img);
        cv::Mat demo_pred2 = draw_vis(draw_img, out_pred2.gurobi_output->getMatrixXfMap().topRows(3), template_edges,
                                      intrinsics, cv::Scalar(0, 255, 255));
        if (is_combined_truth) {
          demo_pred2 = draw_vis(demo_pred2, one_frame_truth, template_edges, intrinsics, cv::Scalar(0, 0, 255));
        }
        video_pred2.write(demo_pred2);
      }
      if (is_no_pred) {
        // cv::Mat demo_no_pred = padding_img(draw_img);
        cv::Mat demo_no_pred = draw_vis(draw_img, out_without_prediction.gurobi_output->getMatrixXfMap().topRows(3),
                                        template_edges, intrinsics, cv::Scalar(0, 255, 255));
        if (is_combined_truth) {
          demo_no_pred = draw_vis(demo_no_pred, one_frame_truth, template_edges, intrinsics, cv::Scalar(0, 0, 255));
        }
      ...
      if (is_truth) {
        ...
        cv::Mat demo_truth = draw_vis(draw_img, one_frame_truth, template_edges, intrinsics, cv::Scalar(0, 255, 255));
