...
static void
_destroy_recv_parts (lcm_udp_t *lcm)
{
...
static int
parse_mc_addr_and_port (const char *str, udp_params_t * params)
{
    if (!str || !strlen (str)) {
    ...
    if (inet_aton (words[0], (struct in_addr*) &params->uc_addr) == 0) {
...
new_argument (gpointer key, gpointer value, gpointer user)
{
    ...
    if (!strcmp ((char *) key, "recv_buf_size")) {
        ...
        char *endptr = NULL;
        params->uc_ttl = strtol ((char *) value, &endptr, 0);
        if (endptr == value)
...
static int
_recv_message_fragment (lcm_udp_t *lcm, lcm_buf_t *lcmb, uint32_t sz)
{
...
static int
_recv_short_message (lcm_udp_t *lcm, lcm_buf_t *lcmb, int sz)
{
...
static int
lcm_udp_get_fileno (lcm_udp_t *lcm)
{
...
static int
lcm_udp_subscribe (lcm_udp_t *lcm, const char *channel)
{
    return _setup_recv_parts (lcm);
}

static int
lcm_udp_publish (lcm_udp_t *lcm, const char *channel, const void *data,
...
udpm_self_test (lcm_udp_t *lcm)
{
    ...
    do {
        ...
        if (lcm_timeval_compare (&now, &next_retransmit) > 0) {
            status = lcm_udp_publish (lcm, SELF_TEST_CHANNEL, (uint8_t*)msg,
                    strlen (msg));
            lcm_timeval_add (&now, &retransmit_interval, &next_retransmit);
        }
...
static int
_setup_recv_parts (lcm_udp_t* lcm)
{
    g_static_rec_mutex_lock(&lcm->mutex);
    ...
    addr.sin_port = lcm->params.uc_port;
...
lcm_provider_t *
lcm_udp_create (lcm_t * parent, const char *network, const GHashTable *args)
{
    udp_params_t params;
