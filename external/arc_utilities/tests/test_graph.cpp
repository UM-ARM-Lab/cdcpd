...
TEST(GraphTestSuite, serialization)
{
    ...
    for(int64_t n_id=0; n_id < (int64_t)test_graph.getNodes().size(); n_id++)
    {
        ...
        {
            EXPECT_EQ(n.getOutEdges()[e_id], other_n.getOutEdges()[e_id]) <<
                "Test and deserialized out edge are different";
        }
        for(int64_t e_id=0; e_id < (int64_t)n.getInEdges().size(); e_id++)
        {
            EXPECT_EQ(n.getInEdges()[e_id], other_n.getInEdges()[e_id]) <<
                "Test and deserialized in edge are different";
        }
    }
}
