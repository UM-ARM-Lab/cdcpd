...
int main(int argc, char* argv[])
{
    ...
    // Testing 2D
    {
        ...
        while (ros::ok())
        {
            ...
            const Matrix2Xd target_points = template_points
    ...
    // Testing 3D
    {
        ...
        for (auto x_ind = 0; x_ind < num_grid_x_points; ++x_ind)
        {
            for (auto y_ind = 0; y_ind < num_grid_y_points; ++y_ind)
            {
                ...
                {
                    grid_points.col((x_ind * num_grid_y_points + y_ind) * num_grid_z_points + z_ind) << X_grid(x_ind), Y_grid(y_ind), Z_grid(z_ind);
                }
            }
        }
        ...
        while (ros::ok())
        {
            ...
            const Matrix3Xd target_points = template_points
                    + 2.0 * Matrix3Xd::Ones(3, num_points)
                    + 0.2 * Matrix3Xd::Random(3, num_points);
