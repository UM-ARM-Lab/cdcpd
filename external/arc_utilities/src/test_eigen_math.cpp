...
inline void TestVector3d(const ssize_t iterations, const Eigen::Isometry3d& base_transform, Eigen::MatrixXd& results) {
  ...
  for (ssize_t idx = 0; idx < iterations; idx++) {
    Eigen::Vector3d test_vector(1.0 * static_cast<double>(idx), 2.0 * static_cast<double>(idx),
                                3.0 * static_cast<double>(idx));
  ...
  std::cout << "Isometry3d * Vector3d test - " << vector3_test_time.count() << "s for " << iterations
            << " iterations to produce " << results.block<4, 1>(100000, 0) << std::endl;
}
...
inline void TestVector4d(const ssize_t iterations, const Eigen::Isometry3d& base_transform, Eigen::MatrixXd& results) {
  ...
  for (ssize_t idx = 0; idx < iterations; idx++) {
    Eigen::Vector4d test_vector(1.0 * static_cast<double>(idx), 2.0 * static_cast<double>(idx),
                                3.0 * static_cast<double>(idx), 1.0);
  ...
  std::cout << "Isometry3d * Vector4d test - " << vector4_test_time.count() << "s for " << iterations
            << " iterations to produce " << results.block<4, 1>(100000, 0) << std::endl;
}

inline void TestAlignedVector3d(const ssize_t iterations, const Eigen::Isometry3d& base_transform,
  ...
  for (ssize_t idx = 0; idx < iterations; idx++) {
    Eigen::Aligned4Vector3<double> test_vector(1.0 * static_cast<double>(idx), 2.0 * static_cast<double>(idx),
                                               3.0 * static_cast<double>(idx));
    results.block<3, 1>(idx * 4, 0) = base_transform * test_vector;
  }
  ...
  std::cout << "Isometry3d * AlignedVector3d test - " << alignedvector3_test_time.count() << "s for " << iterations
            << " iterations to produce " << results.block<4, 1>(100000, 0) << std::endl;
}
...
inline void TestManual(const ssize_t iterations, const Eigen::Isometry3d& base_transform, Eigen::MatrixXd& results) {
  ...
  std::cout << "Matrix4d * Vector4d test - " << manual_test_time.count() << "s for " << iterations
            << " iterations to produce " << results.block<4, 1>(100000, 0) << std::endl;
}
...
int main(int argc, char** argv) {
  ...
  const Eigen::Isometry3d base_transform =
      Eigen::Translation3d(10.0, 10.0, 10.0) * EigenHelpers::QuaternionFromRPY(0.25, 0.5, 0.75);
