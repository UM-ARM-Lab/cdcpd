...
int main(int argc, char** argv) {
  ...
  const EigenHelpers::VectorIsometry3d abb_irb_1600_145_link_transforms =
      ABB_IRB1600_145_FK_FAST::GetLinkTransforms(abb_irb_1600_145_base_config);
  ...
  const EigenHelpers::VectorIsometry3d iiwa_14_link_transforms =
      IIWA_14_FK_FAST::GetLinkTransforms(iiwa_14_base_config);
  ...
  std::cout << "Vector: " << PrettyPrint::PrettyPrint(testvecs[0])
            << " Weighted norm: " << EigenHelpers::WeightedNorm(testvecs[0], weights) << std::endl;
  std::cout << "Vector: " << PrettyPrint::PrettyPrint(testvecs[7])
            << " Weighted norm: " << EigenHelpers::WeightedNorm(testvecs[7], weights) << std::endl;
  std::cout << "Weighted angle between vectors: "
            << EigenHelpers::WeightedAngleBetweenVectors(testvecs[0], testvecs[7], weights) << std::endl;
  std::cout << "Unweighted angle between vectors: "
            << EigenHelpers::WeightedAngleBetweenVectors(testvecs[0], testvecs[7], Eigen::Vector3d::Ones())
            << std::endl;
  std::cout << "Vector: " << PrettyPrint::PrettyPrint(testvecs[1])
            << " Weighted norm: " << EigenHelpers::WeightedNorm(testvecs[1], weights) << std::endl;
  std::cout << "Vector: " << PrettyPrint::PrettyPrint(testvecs[2])
            << " Weighted norm: " << EigenHelpers::WeightedNorm(testvecs[2], weights) << std::endl;
  std::cout << "Weighted angle between vectors: "
            << EigenHelpers::WeightedAngleBetweenVectors(testvecs[1], testvecs[2], weights) << std::endl;
  std::cout << "Unweighted angle between vectors: "
            << EigenHelpers::WeightedAngleBetweenVectors(testvecs[1], testvecs[2], Eigen::Vector3d::Ones())
            << std::endl;
  std::cout << std::endl;
  ...
  std::function<double(const double&, const double&)> double_distance_fn = [](const double& d1, const double& d2) {
  ...
  const double reversed_test_dist =
      simple_dtw::ComputeDTWDistance(test_trunc_normals, reversed_test_trunc_normals, double_distance_fn);
