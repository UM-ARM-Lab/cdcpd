...
std::vector<uint8_t> LoadFromFileAndDecompress(const std::string& path) {
  if (!boost::filesystem::is_regular_file(path)) {
    if (boost::filesystem::is_regular_file(path + "." + std::to_string(0))) {
      std::cerr << "ZLibHelpers::LoadFromFileAndDecompress(): " << path
                << " is not a regular file. Loading from individual slices." << std::endl;
      ...
      do {
        ...
        std::cerr << "Reading bytes [" << decompressed.size() << ", " << decompressed.size() + slice_bytes.size()
                  << ") from " << slice_path << std::endl;
...
void CompressAndWriteToFile(const std::vector<uint8_t>& uncompressed, const std::string& path) {
  ...
  if (uncompressed.size() > max_bytes) {
    std::cerr << "ZLibHelpers::CompressAndWriteToFile(): Input buffer too large for single compression. Splitting into "
                 "multiple files"
              << std::endl;
    int file_idx = 0;
    do {
