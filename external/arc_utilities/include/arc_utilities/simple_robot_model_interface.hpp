...
class SimpleRobotModelInterface {
 public:
  ...
  virtual double ComputeConfigurationDistance(const Configuration& config1, const Configuration& config2) const = 0;
  ...
  virtual Configuration InterpolateBetweenConfigurations(const Configuration& start, const Configuration& end,
  ...
  virtual Eigen::Matrix<double, 3, Eigen::Dynamic> ComputeLinkPointTranslationJacobian(
      const std::string& link_name, const Eigen::Vector4d& link_relative_point) const = 0;
};
}  // namespace simple_robot_model_interface
