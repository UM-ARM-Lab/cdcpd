...
inline Value RetrieveOrDefault(const std::map<Key, Value, Compare, Allocator>& map, const Key& key,
...
template <typename Key, typename Value, typename Hash = std::hash<Key>, typename Predicate = std::equal_to<Key>,
          typename Allocator = std::allocator<std::pair<const Key, Value>>>
inline Value RetrieveOrDefault(const std::unordered_map<Key, Value, Hash, Predicate, Allocator>& map, const Key& key,
                               const Value& default_val) {
  const auto found_itr = map.find(key);
  if (found_itr != map.end()) {
    return found_itr->second;
...
// Written to mimic parts of Matlab wthresh(val, 'h', thresh) behavior, spreading the value to the thresholds instead of
// setting them to zero https://www.mathworks.com/help/wavelet/ref/wthresh.html
template <class T>
...
inline T SpreadValueAndWarn(const T& val, const T& low_threshold, const T& midpoint, const T& high_threshold) {
  ...
  if (val >= midpoint && val < high_threshold) {
    const std::string msg =
        "Thresholding " + std::to_string(val) + " to high threshold " + std::to_string(high_threshold) + "\n";
    std::cerr << msg << std::flush;
    return high_threshold;
  } else if (val < midpoint && val > low_threshold) {
    const std::string msg =
        "Thresholding " + std::to_string(val) + " to low threshold " + std::to_string(low_threshold) + "\n";
    std::cerr << msg << std::flush;
    return low_threshold;
  }
  return val;
...
class SplitMix64PRNG {
 ...
 public:
  inline SplitMix64PRNG(const uint64_t seed_val) { seed(seed_val); }

  static constexpr uint64_t min(void) { return 0u; }

  static constexpr uint64_t max(void) { return std::numeric_limits<uint64_t>::max(); }

  inline void seed(const uint64_t seed_val) { state_ = seed_val; }
  ...
  inline uint64_t operator()(void) { return next(); }
};
...
class XorShift128PlusPRNG {
 ...
 public:
  inline XorShift128PlusPRNG(const uint64_t seed_val) { seed(seed_val); }
...
inline double EvaluateTruncatedGaussianCDF(const double mean, const double lower_bound, const double upper_bound,
...
inline double EvaluateTruncatedGaussianPDF(const double mean, const double lower_bound, const double upper_bound,
                                           const double std_dev, const double val) {
  assert(lower_bound <= upper_bound);
  if (val <= lower_bound) {
    return 0.0;
...
inline double IntegrateGaussian(const double mean, const double std_dev, const double lower_limit,
  ...
  // const std::string msg = "Integrated Gaussian with mean " + std::to_string(mean) + " std.dev. " +
  // std::to_string(std_dev) + " over range [" + std::to_string(lower_limit) + "," + std::to_string(upper_limit) + "] to
  // be " + std::to_string(probability); std::cout << msg << std::endl;
  return probability;
}

inline double IntegrateTruncatedGaussian(const double mean, const double lower_bound, const double upper_bound,
  ...
  const double probability = upper_limit_cdf - lower_limit_cdf;
  // const std::string msg = "Integrated truncated Gaussian with mean " + std::to_string(mean) + " std.dev. " +
  // std::to_string(std_dev) + " and bounds [" + std::to_string(lower_bound) + "," + std::to_string(upper_bound) + "]
  // over range [" + std::to_string(lower_limit) + "," + std::to_string(upper_limit) + "] to be " +
  // std::to_string(probability); std::cout << msg << std::endl;
  return probability;
}
...
class TruncatedNormalDistribution {
 protected:
  ...
  inline double NaiveAcceptReject(const double lower_bound, const double upper_bound, Generator& prng) {
    while (true) {
      const double draw =
          normal_dist_(prng);  // Rf_rnorm(0.0, 1.0) ; // Normal distribution (i.e. std::normal_distribution<double>)
  ...
  inline double ComplexAcceptReject(const double lower_bound, const double upper_bound, Generator& prng) {
    while (true) {
      const double z = uniform_range_dist_(prng);  // Rf_runif(lower_bound, upper_bound) ; // Uniform distribution (i.e.
  ...
  inline double Sample(Generator& prng) {
    if (case_ == TYPE_1) {
      ...
      while (true) {
        const double draw =
            SimpleAcceptReject(std_lower_bound_, prng);  // use the simple algorithm if it is more efficient
 ...
 public:
  inline TruncatedNormalDistribution(const double mean, const double stddev, const double lower_bound,
...
class MultivariteGaussianDistribution {
 ...
 public:
  inline MultivariteGaussianDistribution(const Eigen::VectorXd& mean, const Eigen::MatrixXd& covariance)
      : mean_(mean), norm_transform_(CalculateNormTransform(covariance)), unit_gaussian_dist_(0.0, 1.0) {
...
class RandomRotationGenerator {
 ...
 public:
  ...
  static inline Eigen::Vector3d GenerateUniformRandomEulerAngles(const std::function<double()>& uniform_unit_dist) {
    ...
    const double pitch =
        (uniform_unit_dist() < 0.5) ? ((pitch_init < M_PI) ? pitch_init + M_PI : pitch_init - M_PI) : pitch_init;
...
template <typename Key, typename Value, typename Compare = std::less<Key>,
          typename KeyVectorAllocator = std::allocator<Key>, typename ValueVectorAllocator = std::allocator<Value>,
...
inline std::map<Key, Value, Compare, PairAllocator> MakeFromKeysAndValues(
    const std::vector<Key, KeyVectorAllocator>& keys, const std::vector<Value, ValueVectorAllocator>& values) {
...
inline void Sleep(const double time) { std::this_thread::sleep_for(std::chrono::duration<double>(time)); }
}  // namespace arc_helpers
