...
#pragma omp parallel
  ...
  static std::pair<std::pair<std::pair<bool, int64_t>, std::pair<bool, int64_t>>, double> GetClosestPair(
      const std::vector<uint8_t>& datapoint_mask, const Eigen::MatrixXd& distance_matrix,
      const std::vector<std::vector<int64_t>>& clusters, const CLUSTER_STRATEGY strategy) {
    // Compute distances between unclustered points <-> unclustered points, unclustered_points <-> clusters, and
    // clusters <-> clusters Compute the minimum unclustered point <-> unclustered point / unclustered_point <-> cluster
    // distance
#ifdef ENABLE_PARALLEL_COMPLETE_LINK_CLUSTERING
    const size_t num_threads = GetNumOMPThreads();
    std::vector<double> per_thread_min_distances(num_threads, INFINITY);
    std::vector<std::pair<int64_t, std::pair<bool, int64_t>>> per_thread_min_element_pairs(
        num_threads, std::make_pair(-1, std::make_pair(false, -1)));
#pragma omp parallel for
    ...
    for (size_t idx = 0; idx < datapoint_mask.size(); idx++) {
      ...
      if (datapoint_mask[idx] == 0) {
        ...
        std::pair<int64_t, std::pair<bool, int64_t>>& per_thread_min_element_pair =
            per_thread_min_element_pairs[thread_num];
...
#pragma omp parallel for
        ...
        for (size_t scdx = 0; scdx < clusters.size(); scdx++) {
          ...
          if (fcdx != scdx) {
            ...
            if (second_cluster.size() > 0) {
              ...
              const double cluster_cluster_distance =
                  (strategy == COMPLETE_LINK) ? complete_link_distance : single_link_distance;
#ifdef ENABLE_PARALLEL_COMPLETE_LINK_CLUSTERING
              const size_t thread_num = (size_t)omp_get_thread_num();
    ...
    if (min_distance < min_cluster_cluster_distance) {
      ...
      const std::pair<std::pair<std::pair<bool, int64_t>, std::pair<bool, int64_t>>, double> minimum_pair(indices,
                                                                                                          min_distance);
      return minimum_pair;
    }
    // A cluster <-> cluster pair is closest
      ...
      const std::pair<std::pair<bool, int64_t>, std::pair<bool, int64_t>> indices(first_index, second_index);
      const std::pair<std::pair<std::pair<bool, int64_t>, std::pair<bool, int64_t>>, double> minimum_pair(
          indices, min_cluster_cluster_distance);
      return minimum_pair;
    }
  }
 ...
 public:
  ...
  static std::pair<std::vector<std::vector<Datatype, Allocator>>, double> Cluster(
      const std::vector<Datatype, Allocator>& data,
      const std::function<double(const Datatype&, const Datatype&)>& distance_fn, const double max_cluster_distance,
      const CLUSTER_STRATEGY strategy) {
#ifdef ENABLE_PARALLEL_COMPLETE_LINK_CLUSTERING
  ...
  template <typename Datatype, typename Allocator = std::allocator<Datatype>>
  static std::pair<std::vector<std::vector<Datatype, Allocator>>, double> Cluster(
      const std::vector<Datatype, Allocator>& data, const Eigen::MatrixXd& distance_matrix,
      const double max_cluster_distance, const CLUSTER_STRATEGY strategy) {
    ...
    while (!complete) {
      ...
      const std::pair<std::pair<std::pair<bool, int64_t>, std::pair<bool, int64_t>>, double> closest_element_pair =
          GetClosestPair(datapoint_mask, distance_matrix, cluster_indices, strategy);
      const std::pair<std::pair<bool, int64_t>, std::pair<bool, int64_t>>& closest_elements =
          closest_element_pair.first;
