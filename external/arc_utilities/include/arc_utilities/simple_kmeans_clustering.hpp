...
class SimpleKMeansClustering {
 private:
  ...
  static uint32_t GetClosestCluster(const Datatype& datapoint,
  ...
  static std::vector<Datatype, Allocator> ComputeClusterCenters(
      const std::vector<Datatype, Allocator>& data, const std::vector<uint32_t>& cluster_labels,
      const std::function<Datatype(const std::vector<Datatype, Allocator>&)>& average_fn, const uint32_t num_clusters) {
  ...
  static std::vector<Datatype, Allocator> ComputeClusterCentersWeighted(
      const std::vector<Datatype, Allocator>& data, const std::vector<double>& weights,
  ...
  static bool CheckForConvergence(const std::vector<uint32_t>& old_labels, const std::vector<uint32_t>& new_labels) {
 ...
 public:
  ...
  static std::vector<uint32_t> Cluster(
    ...
    if (num_clusters == 1) {
      std::cerr << "[K-means clustering] Provided num_clusters = 1, returning default labels for cluster 0"
                << std::endl;
    ...
    if (enable_preliminary_clustering) {
      ...
      if (subset_size >= (num_clusters * 5)) {
        ...
        std::cerr << "[K-means clustering] Preliminary clustering enabled, using subset of " << subset_size
                  << " datapoints from " << data.size() << " total" << std::endl;
      } else {
        enable_preliminary_clustering = false;
        std::cerr << "[K-means clustering] Preliminary clustering disabled as input data is too small w.r.t. number of "
                     "clusters"
                  << std::endl;
      }
    }
  ...
  static std::vector<uint32_t> ClusterWeighted(
      ...
      const std::function<double(const Datatype&, const Datatype&)>& distance_fn,
      const std::function<Datatype(const std::vector<Datatype, Allocator>&, const std::vector<double>&)>& average_fn,
      const uint32_t num_clusters, const bool do_preliminary_clustering = false) {
    assert(data.size() > 0);
    assert(num_clusters > 0);
    ...
    if (enable_preliminary_clustering) {
      ...
      assert(random_subset.size() == subset_size);
      // Run clustering on the subset
      std::vector<uint32_t> random_subset_labels =
          ClusterWeighted(random_subset, weights, distance_fn, average_fn, num_clusters, false);
      // Now we use the centers of the clusters to form the cluster centers
      cluster_centers =
          ComputeClusterCentersWeighted(random_subset, weights, random_subset_labels, average_fn, num_clusters);
    } else {
      // This makes sure we don't get duplicates
      std::map<size_t, uint8_t> index_map;
      while (index_map.size() < num_clusters) {
