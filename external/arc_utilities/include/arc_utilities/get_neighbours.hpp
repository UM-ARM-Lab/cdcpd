...
inline std::vector<ConfigType, Allocator> TwoDimensional8Connected(
    const ConfigType& config, const StepType& min_x, const StepType& max_x, const StepType& min_y,
    const StepType& max_y, const StepType& step_size,
  ...
  for (int x_offset = -1; x_offset <= 1; ++x_offset) {
    ...
    for (int y_offset = -1; y_offset <= 1; ++y_offset) {
      ...
      if (min_vector[0] <= neighbour[0] && neighbour[0] <= max_vector[0] && min_vector[1] <= neighbour[1] &&
          neighbour[1] <= max_vector[1] && (neighbour[0] != config[0] || neighbour[1] != config[1]) &&
...
inline std::vector<ConfigType, Allocator> ThreeDimensional8Connected(
    const ConfigType& config, const StepType& min_x, const StepType& max_x, const StepType& min_y,
    const StepType& max_y, const StepType& min_z, const StepType& max_z, const StepType& step_size,
    const std::function<ConfigType(const ConfigType&)>& round_to_grid_fn,
    const std::function<bool(const ConfigType&)>& validity_check_fn) {
  std::vector<ConfigType, Allocator> neighbours;
  ...
  for (int x_offset = -1; x_offset <= 1; ++x_offset) {
    ...
    for (int y_offset = -1; y_offset <= 1; ++y_offset) {
      ...
      for (int z_offset = -1; z_offset <= 1; ++z_offset) {
        ...
        if (min_vector[0] <= neighbour[0] && neighbour[0] <= max_vector[0] && min_vector[1] <= neighbour[1] &&
            neighbour[1] <= max_vector[1] && min_vector[2] <= neighbour[2] && neighbour[2] <= max_vector[2] &&
