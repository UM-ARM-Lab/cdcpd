...
class SimpleAStarPlanner {
 protected:
  ...
  struct AStarComparator {
   public:
    AStarComparator(const std::function<double(const ConfigType&)>& heuristic_fn) : heuristic_fn_(heuristic_fn) {}

    // Defines a "less" operation"; by using "greater" then the smallest element will appear at the top of the priority
    // queue
    bool operator()(const ConfigAndDistType& c1, const ConfigAndDistType& c2) const {
      // If both expected distances are the same, then we want to explore the one that has the smaller heuristic
      // distance
  ...
  static std::vector<ConfigType, Allocator> ExtractPathBasic(
      const std::unordered_map<ConfigType, ConfigType, ConfigHasher>& backpointers, ConfigType last_state) {
    std::vector<ConfigType, Allocator> path;
    for (auto backpointer_ittr = backpointers.find(last_state); backpointer_ittr != backpointers.end();
         backpointer_ittr = backpointers.find(last_state)) {
 ...
 public:
  ...
  static std::pair<std::vector<ConfigType, Allocator>, std::map<std::string, double>> Plan(
      const ConfigType& start, const std::function<std::vector<ConfigType, Allocator>(const ConfigType&)>& neighbour_fn,
    ...
    //                marker.color = arc_helpers::RGBAColorBuilder<std_msgs::ColorRGBA>::MakeFromFloatColors(1.0, 1.0,
    //                0.0, 1.0);
    ...
    //                neighbours_marker.color =
    //                arc_helpers::RGBAColorBuilder<std_msgs::ColorRGBA>::MakeFromFloatColors(0.0, 0.0, 1.0, 0.3);
    ...
    while (!goal_reached && frontier.size() > 0) {
      ...
      //    so we want to avoid the expense of re-exploring it, and just discard this one once we pop it
        ...
        //                        neighbours_marker.points =
        //                        EigenHelpersConversions::VectorEigenVector3dToVectorGeometryPoint(neighbours);
        ...
        for (const auto neighbour : neighbours) {
          ...
          if (neighbour_cost_to_come_ittr != cost_to_come.end() &&
              neighbour_cost_to_come_ittr->second <= neighbour_new_cost_to_come) {
            continue;
          }
