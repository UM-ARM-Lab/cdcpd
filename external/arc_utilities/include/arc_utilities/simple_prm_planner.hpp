...
class SimpleGeometricPrmPlanner {
 protected:
  ...
  static int64_t AddNodeToRoadmap(const T& state, const NNDistanceDirection nn_distance_direction,
                                  arc_dijkstras::Graph<T, Allocator>& roadmap,
                                  const std::function<double(const T&, const T&)>& distance_fn,
                                  const std::function<bool(const T&, const T&)>& edge_validity_check_fn, const size_t K,
                                  const bool distance_is_symmetric = true, const bool add_duplicate_states = false) {
...
#pragma omp parallel for
  ...
  template <typename T, typename Allocator = std::allocator<T>>
  static std::vector<T, Allocator> ExtractSolutionPath(const arc_dijkstras::Graph<T, Allocator>& roadmap,
 ...
 public:
  template <typename T, typename Allocator = std::allocator<T>>
  static void ExtendRoadMap(arc_dijkstras::Graph<T, Allocator>& roadmap, const std::function<T(void)>& sampling_fn,
                            const std::function<double(const T&, const T&)>& distance_fn,
                            const std::function<bool(const T&)>& state_validity_check_fn,
                            const std::function<bool(const T&, const T&)>& edge_validity_check_fn,
                            const std::function<bool(void)>& termination_check_fn, const size_t K,
                            const bool distance_is_symmetric = true, const bool add_duplicate_states = false) {
    while (!termination_check_fn()) {
      const T random_state = sampling_fn();
      if (state_validity_check_fn(random_state)) {
        AddNodeToRoadmap(random_state, ROADMAP_TO_NEW_STATE, roadmap, distance_fn, edge_validity_check_fn, K,
  ...
  static arc_dijkstras::Graph<T, Allocator> BuildRoadMap(
      const std::function<T(void)>& sampling_fn, const std::function<double(const T&, const T&)>& distance_fn,
      const std::function<bool(const T&)>& state_validity_check_fn,
      const std::function<bool(const T&, const T&)>& edge_validity_check_fn,
      const std::function<bool(void)>& termination_check_fn, const size_t K, const bool distance_is_symmetric = true,
      const bool add_duplicate_states = false) {
    arc_dijkstras::Graph<T, Allocator> roadmap;
    ExtendRoadMap(roadmap, sampling_fn, distance_fn, state_validity_check_fn, edge_validity_check_fn,
                  termination_check_fn, K, distance_is_symmetric, add_duplicate_states);
    return roadmap;
  }

  template <typename T, typename Allocator = std::allocator<T>>
  static void UpdateRoadMapEdges(arc_dijkstras::Graph<T, Allocator>& roadmap,
                                 const std::function<bool(const T&, const T&)>& edge_validity_check_fn,
                                 const std::function<double(const T&, const T&)>& distance_fn) {
...
#pragma omp parallel for
  ...
  static std::pair<std::vector<T, Allocator>, double> QueryPathAndAddNodesMultiStartSingleGoal(
      const std::vector<T, Allocator>& starts, const T& goal, arc_dijkstras::Graph<T, Allocator>& roadmap,
      const std::function<bool(const T&, const T&)>& edge_validity_check_fn,
      const std::function<double(const T&, const T&)>& distance_fn, const size_t K,
      const bool distance_is_symmetric = true, const bool add_duplicate_states = false) {
    ...
    for (size_t start_idx = 0; start_idx < starts.size(); start_idx++) {
      ...
      start_node_indices[start_idx] =
          AddNodeToRoadmap(start, NEW_STATE_TO_ROADMAP, roadmap, distance_fn, edge_validity_check_fn, K,
                           distance_is_symmetric, add_duplicate_states);
    }
    // Add the goal node to the roadmap
    const int64_t goal_node_index =
        AddNodeToRoadmap(goal, ROADMAP_TO_NEW_STATE, roadmap, distance_fn, edge_validity_check_fn, K,
                         distance_is_symmetric, add_duplicate_states);
    // Call Dijkstra's
    const auto dijkstras_solution =
        arc_dijkstras::SimpleDijkstrasAlgorithm<T, Allocator>::PerformDijkstrasAlgorithm(roadmap, goal_node_index);
  ...
  static std::pair<std::vector<T, Allocator>, double> QueryPathAndAddNodesSingleStartSingleGoal(
      const T& start, const T& goal, arc_dijkstras::Graph<T, Allocator>& roadmap,
      const std::function<bool(const T&, const T&)>& edge_validity_check_fn,
      const std::function<double(const T&, const T&)>& distance_fn, const size_t K,
      const bool distance_is_symmetric = true, const bool add_duplicate_states = false,
    ...
    const std::pair<std::vector<int64_t>, double> astar_result =
        arc_dijkstras::SimpleGraphAstar<T, Allocator>::PerformAstar(roadmap, start_node_index, goal_node_index,
                                                                    distance_fn, limit_astar_pqueue_duplicates);
  ...
  static std::pair<std::vector<T, Allocator>, double> LazyQueryPathAndAddNodesSingleStartSingleGoal(
    ...
    // Call graph A*
    const std::pair<std::vector<int64_t>, double> astar_result =
        arc_dijkstras::SimpleGraphAstar<T, Allocator>::PerformLazyAstar(roadmap, start_node_index, goal_node_index,
                                                                        edge_validity_check_fn, distance_fn,
                                                                        distance_fn, limit_astar_pqueue_duplicates);
    // Convert the solution path from A* provided as indices into real states
  ...
  static std::pair<std::vector<T, Allocator>, double> QueryPathAndAddNodesSingleStartSingleGoalRandomWalk(
      const T& start, const T& goal, Generator& generator, arc_dijkstras::Graph<T, Allocator>& roadmap,
      const std::function<bool(const T&, const T&)>& edge_validity_check_fn,
      const std::function<double(const T&, const T&)>& distance_fn, const size_t K,
      const bool distance_is_symmetric = true) {
    // Add the start node to the roadmap
    const int64_t start_node_index = AddNodeToRoadmap(start, NEW_STATE_TO_ROADMAP, roadmap, distance_fn,
                                                      edge_validity_check_fn, K, distance_is_symmetric);
    // Add the goal node to the roadmap
    const int64_t goal_node_index = AddNodeToRoadmap(goal, ROADMAP_TO_NEW_STATE, roadmap, distance_fn,
                                                     edge_validity_check_fn, K, distance_is_symmetric);
    // Call the random walk algorithm
    const auto random_walk_result = arc_dijkstras::GraphRandomWalk<T, Allocator>::PerformRandomWalk(
        roadmap, start_node_index, goal_node_index, generator);
  ...
  // TODO - figure out a better way to balance parallelism between KNN queries inside path calls and multiple calls to
  // Dijkstras
  template <typename T, typename Allocator = std::allocator<T>>
  static std::pair<std::vector<T, Allocator>, double> QueryPathMultiStartMultiGoal(
      const std::vector<T, Allocator>& starts, const std::vector<T, Allocator>& goals,
    ...
    for (size_t goal_idx = 0; goal_idx < goals.size(); goal_idx++) {
      possible_solutions[goal_idx] =
          QueryPathMultiStartSingleGoal(starts, goals[goal_idx], roadmap, edge_validity_check_fn, distance_fn, K,
                                        distance_is_symmetric, add_duplicate_states);
    }
  ...
  static std::pair<std::vector<T, Allocator>, double> QueryPathMultiStartSingleGoal(
      const std::vector<T, Allocator>& starts, const T& goal, const arc_dijkstras::Graph<T, Allocator>& roadmap,
      const std::function<bool(const T&, const T&)>& edge_validity_check_fn,
      const std::function<double(const T&, const T&)>& distance_fn, const size_t K,
      const bool distance_is_symmetric = true, const bool add_duplicate_states = false) {
    arc_dijkstras::Graph<T, Allocator> working_copy = roadmap;
    return QueryPathAndAddNodesMultiStartSingleGoal(starts, goal, working_copy, edge_validity_check_fn, distance_fn, K,
                                                    distance_is_symmetric, add_duplicate_states);
  }
  ...
  static std::pair<std::vector<T, Allocator>, double> QueryPathSingleStartSingleGoal(
      ...
      const bool limit_astar_pqueue_duplicates = true) {
    arc_dijkstras::Graph<T, Allocator> working_copy = roadmap;
    return QueryPathAndAddNodesSingleStartSingleGoal(start, goal, working_copy, edge_validity_check_fn, distance_fn, K,
                                                     distance_is_symmetric, add_duplicate_states,
  ...
  static std::pair<std::vector<T, Allocator>, double> LazyQueryPathSingleStartSingleGoal(
      ...
      const bool limit_astar_pqueue_duplicates = true) {
    arc_dijkstras::Graph<T, Allocator> working_copy = roadmap;
    return LazyQueryPathAndAddNodesSingleStartSingleGoal(start, goal, working_copy, edge_validity_check_fn, distance_fn,
                                                         K, distance_is_symmetric, add_duplicate_states,
                                                         limit_astar_pqueue_duplicates);
  }
};
}  // namespace simple_prm_planner
