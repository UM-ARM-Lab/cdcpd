...
class SimpleSE3ConfigSerializer {
 public:
  static inline std::string TypeName() { return std::string("EigenIsometry3dSerializer"); }
  ...
  static inline std::pair<SimpleSE3Configuration, uint64_t> Deserialize(const std::vector<uint8_t>& buffer,
...
class SimpleSE3Robot
    : public simple_robot_model_interface::SimpleRobotModelInterface<SimpleSE3Configuration, SimpleSE3ConfigAlloc> {
 protected:
  ...
  inline void SetConfig(const Eigen::Isometry3d& new_config) { config_ = new_config; }

 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  SimpleSE3Robot(const SimpleSE3Configuration& initial_position, const double position_distance_weight,
  ...
  virtual simple_robot_model_interface::SimpleRobotModelInterface<SimpleSE3Configuration, SimpleSE3ConfigAlloc>* Clone()
      const {
    return new SimpleSE3Robot(static_cast<const SimpleSE3Robot&>(*this));
  }

  inline const SimpleSE3Configuration& GetPosition() const { return config_; }
  ...
  virtual Eigen::VectorXd ComputePerDimensionConfigurationSignedDistance(const SimpleSE3Configuration& config1,
                                                                         const SimpleSE3Configuration& config2) const {
  ...
  virtual double ComputeConfigurationDistance(const SimpleSE3Configuration& config1,
                                              const SimpleSE3Configuration& config2) const {
  ...
  virtual Eigen::Isometry3d InterpolateBetweenConfigurations(const SimpleSE3Configuration& start,
                                                             const SimpleSE3Configuration& end,
                                                             const double ratio) const {
    return EigenHelpers::Interpolate(start, end, ratio);
  }

  virtual Eigen::Isometry3d AverageConfigurations(
      const std::vector<SimpleSE3Configuration, SimpleSE3ConfigAlloc>& configurations) const {
