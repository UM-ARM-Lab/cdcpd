...
class PointSphereGeometry {
 ...
 public:
  PointSphereGeometry(const MODEL_GEOMETRY_TYPE geometry_type,
      ...
      : geometry_type_(geometry_type), geometry_(geometry) {}
...
class MeshGeometry {
 ...
 public:
  ...
  MeshGeometry(const Eigen::Isometry3d& origin_transform, const Eigen::Vector3d& scale, const MeshType& mesh)
      : origin_transform_(origin_transform), scale_(scale), mesh_(mesh) {}

  MeshGeometry() : origin_transform_(Eigen::Isometry3d::Identity()), scale_(Eigen::Vector3d(0.0, 0.0, 0.0)) {}
...
class SE2RobotWithGeometry : public simple_se2_robot_model::SimpleSE2Robot, RobotWithGeometry<GeometryType> {
 ...
 public:
  SE2RobotWithGeometry(const simple_se2_robot_model::SimpleSE2Configuration& initial_position,
                       const double position_distance_weight, const double rotation_distance_weight,
                       const std::string& link_name, const GeometryType& geometry)
      : simple_se2_robot_model::SimpleSE2Robot(initial_position, position_distance_weight, rotation_distance_weight,
                                               link_name),
        RobotWithGeometry<GeometryType>() {
    link_geometries_.push_back(std::make_pair(link_name, geometry));
  }

  virtual simple_robot_model_interface::SimpleRobotModelInterface<simple_se2_robot_model::SimpleSE2Configuration,
                                                                  simple_se2_robot_model::SimpleSE2ConfigAlloc>*
  Clone() const {
    return new SE2RobotWithGeometry<GeometryType>(static_cast<const SE2RobotWithGeometry<GeometryType>&>(*this));
  }

  inline const std::vector<std::pair<std::string, GeometryType>>& GetLinkGeometries() const { return link_geometries_; }
...
class SE3RobotWithGeometry : public simple_se3_robot_model::SimpleSE3Robot, RobotWithGeometry<GeometryType> {
 ...
 public:
  SE3RobotWithGeometry(const simple_se3_robot_model::SimpleSE3Configuration& initial_position,
                       const double position_distance_weight, const double rotation_distance_weight,
                       const std::string& link_name, const GeometryType& geometry)
      : simple_se3_robot_model::SimpleSE3Robot(initial_position, position_distance_weight, rotation_distance_weight,
                                               link_name),
        RobotWithGeometry<GeometryType>() {
    link_geometries_.push_back(std::make_pair(link_name, geometry));
  }

  virtual simple_robot_model_interface::SimpleRobotModelInterface<simple_se3_robot_model::SimpleSE3Configuration,
                                                                  simple_se3_robot_model::SimpleSE3ConfigAlloc>*
  Clone() const {
...
class LinkedRobotWithGeometry : public simple_linked_robot_model::SimpleLinkedRobot, RobotWithGeometry<GeometryType> {
 ...
 public:
  LinkedRobotWithGeometry(const Eigen::Isometry3d& base_transform,
      ...
      : simple_linked_robot_model::SimpleLinkedRobot(base_transform, links, joints, initial_position,
  ...
  virtual simple_robot_model_interface::SimpleRobotModelInterface<simple_linked_robot_model::SimpleLinkedConfiguration,
                                                                  simple_linked_robot_model::SimpleLinkedConfigAlloc>*
  Clone() const {
  ...
  inline bool CheckIfSelfCollisionAllowed(const size_t link1_index, const size_t link2_index) const {
    if (link1_index < links_.size() && link2_index < links_.size()) {
      ...
      throw std::invalid_argument("Query link indices (" + std::to_string(link1_index) + ", " +
                                  std::to_string(link2_index) + " out of range for robot with " +
                                  std::to_string(links_.size()) + " links");
    }
  }

  static inline Eigen::MatrixXi GenerateAllowedSelfColllisionMap(
      const size_t num_links, const std::vector<std::pair<size_t, size_t>>& allowed_self_collisions) {
