...
class SimpleRRTPlannerState {
 ...
 public:
  ...
  static uint64_t Serialize(const SimpleRRTPlannerState<T, Allocator>& state, std::vector<uint8_t>& buffer,
  ...
  static std::pair<SimpleRRTPlannerState<T, Allocator>, uint64_t> Deserialize(
  ...
  SimpleRRTPlannerState() : parent_index_(-1), initialized_(false) { child_indices_.clear(); }

  SimpleRRTPlannerState(const T& value, const int64_t parent_index, const std::vector<int64_t>& child_indices)
      : value_(value), child_indices_(child_indices), parent_index_(parent_index), initialized_(true) {}
  ...
  explicit SimpleRRTPlannerState(const T& value) : value_(value), parent_index_(-1), initialized_(true) {
    child_indices_.clear();
  }
  ...
  uint64_t DeserializeSelf(
    ...
    const std::pair<std::vector<int64_t>, uint64_t> child_indices_deserialized =
        arc_utilities::DeserializeVector<int64_t>(buffer, current_position,
  ...
  void SetParentIndex(const int64_t parent_index) { parent_index_ = parent_index; }

  const std::vector<int64_t>& GetChildIndices() const { return child_indices_; }
...
class SimpleHybridRRTPlanner {
 ...
 public:
   ...
   * goal_reached_fn - return if a given state meets the goal conditions (for example, within a radius of the goal
   * state) state_sampling_fn - returns a new state (randomly- or deterministically-sampled) forward_propagation_fn -
   * given the nearest neighbor and a new target state, returns the states that would grow the tree towards the target
   ...
   * statistics - map of string keys/double values of planner statistics (i.e. run time, #states explored, #states in
   * solution
   */
  ...
  static std::pair<std::vector<T, Allocator>, std::map<std::string, double>> Plan(
      ...
      const std::function<int64_t(const std::vector<SimpleRRTPlannerState<T, Allocator>>&, const T&)>&
          nearest_neighbor_fn,
      const std::function<bool(const T&)>& goal_reached_fn, const std::function<T(void)>& state_sampling_fn,
      const std::function<std::vector<std::pair<T, int64_t>>(const T&, const T&)>& forward_propagation_fn,
      const double goal_bias, const std::chrono::duration<double>& time_limit, RNG& rng) {
    std::uniform_real_distribution<double> goal_bias_distribution(0.0, 1.0);
    const std::function<T(void)> sampling_function = [&](void) {
      return ((goal_bias_distribution(rng) > goal_bias) ? state_sampling_fn() : goal);
    };
   ...
   * forward_propagation_fn - given the nearest neighbor and a new target state, returns the states that would grow the
   * tree towards the target goal_bias - in (0, 1), selects the probability that the new sampled state is a goal state
   * time_limit - limit, in seconds, for the runtime of the planner
   * rng - a random number generator matching the interface of the generators provided by std::random
   *
   * Returns:
  ...
  static std::pair<std::vector<T, Allocator>, std::map<std::string, double>> Plan(
    ...
    std::uniform_real_distribution<double> goal_bias_distribution(0.0, 1.0);
    const std::function<T(void)> sampling_function = [&](void) {
      return ((goal_bias_distribution(rng) > goal_bias) ? state_sampling_fn() : goal_sampling_fn());
    };
    return Plan(start, nearest_neighbor_fn, goal_reached_fn, sampling_function, forward_propagation_fn, time_limit);
  }
   ...
   * state_sampling_fn - returns a new state (randomly- or deterministically-sampled)
   * forward_propagation_fn - given the nearest neighbor and a new target state, returns the states that would grow the
   * tree towards the target time_limit - limit, in seconds, for the runtime of the planner
   *
   * Returns:
   * std::pair<path, statistics>
   * path - vector of states corresponding to the planned path
  ...
  static std::pair<std::vector<T, Allocator>, std::map<std::string, double>> Plan(
      ...
      const std::function<int64_t(const std::vector<SimpleRRTPlannerState<T, Allocator>>&, const T&)>&
          nearest_neighbor_fn,
      const std::function<bool(const T&)>& goal_reached_fn, const std::function<T(void)>& sampling_fn,
      const std::function<std::vector<std::pair<T, int64_t>>(const T&, const T&)>& forward_propagation_fn,
      const std::chrono::duration<double>& time_limit) {
    std::chrono::time_point<std::chrono::steady_clock> start_time = std::chrono::steady_clock::now();
    const std::function<bool(void)> termination_check_fn = [&](void) {
      return (((std::chrono::time_point<std::chrono::steady_clock>)std::chrono::steady_clock::now() - start_time) >
              time_limit);
    };
   ...
   * state_sampling_fn - returns a new state (randomly- or deterministically-sampled)
   * forward_propagation_fn - given the nearest neighbor and a new target state, returns the states that would grow the
   * tree towards the target termination_check_fn - returns if the planner should terminate (for example, if it has
   * exceeded time/space limits)
   *
   * Returns:
   * std::pair<path, statistics>
   * path - vector of states corresponding to the planned path
  ...
  static std::pair<std::vector<T, Allocator>, std::map<std::string, double>> Plan(
    ...
    const std::function<bool(const T&)> real_goal_found_fn = [&](const T& state) {
      if (goal_reached_fn(state)) {
        solution_found = true;
        return true;
      } else {
        return false;
    ...
    const std::function<bool(void)> real_termination_check_fn = [&](void) {
      if (!solution_found) {
        return termination_check_fn();
      } else {
        return true;
      }
    };
    const std::function<void(std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t)>
        dummy_goal_reached_callback_fn = [](std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t) { ; };
    // Call the planner
    std::pair<std::vector<std::vector<T, Allocator>>, std::map<std::string, double>> planning_result =
        PlanMultiPath(start, nearest_neighbor_fn, real_goal_found_fn, dummy_goal_reached_callback_fn, sampling_fn,
                      forward_propagation_fn, real_termination_check_fn);
  ...
  template <typename T, typename Allocator = std::allocator<T>>
  static std::pair<std::vector<T, Allocator>, std::map<std::string, double>> Plan(
      std::vector<SimpleRRTPlannerState<T, Allocator>>& nodes, const T& start,
      const std::function<int64_t(const std::vector<SimpleRRTPlannerState<T, Allocator>>&, const T&)>&
          nearest_neighbor_fn,
      const std::function<bool(const T&)>& goal_reached_fn, const std::function<T(void)>& sampling_fn,
      const std::function<std::vector<std::pair<T, int64_t>>(const T&, const T&)>& forward_propagation_fn,
    ...
    // Call the planner
    std::pair<std::vector<std::vector<T, Allocator>>, std::map<std::string, double>> planning_result =
        PlanMultiPath(nodes, start, nearest_neighbor_fn, real_goal_found_fn, dummy_goal_reached_callback_fn,
                      sampling_fn, forward_propagation_fn, real_termination_check_fn);
    // Put together the return
    std::vector<T, Allocator> planned_path;
    if (planning_result.first.size() > 0) {
      planned_path = planning_result.first[0];
  ...
  static std::pair<std::vector<std::vector<T, Allocator>>, std::map<std::string, double>> PlanMultiPath(
    ...
    return PlanMultiPath(nodes, start, nearest_neighbor_fn, goal_reached_fn, goal_reached_callback_fn, sampling_fn,
                         forward_propagation_fn, termination_check_fn);
  }
  ...
  static std::pair<std::vector<std::vector<T, Allocator>>, std::map<std::string, double>> PlanMultiPath(
    ...
    // Call the planner
    return PlanMultiPath(nodes, nearest_neighbor_fn, goal_reached_fn, goal_reached_callback_fn, sampling_fn,
                         forward_propagation_fn, termination_check_fn);
  }
  ...
  static std::pair<std::vector<std::vector<T, Allocator>>, std::map<std::string, double>> PlanMultiPath(
    ...
    // Call the planner
    return PlanMultiPath(nodes, nearest_neighbor_fn, dummy_state_added_fn, goal_reached_fn, goal_reached_callback_fn,
                         sampling_fn, forward_propagation_fn, termination_check_fn);
  }
  ...
  static std::pair<std::vector<std::vector<T, Allocator>>, std::map<std::string, double>> PlanMultiPath(
      ...
      const std::function<int64_t(const std::vector<SimpleRRTPlannerState<StateType, StateAllocator>>&,
                                  const SampleType&)>& nearest_neighbor_fn,
      const std::function<void(std::vector<SimpleRRTPlannerState<StateType, StateAllocator>>&, const int64_t)>&
          state_added_fn,
      const std::function<bool(const StateType&)>& goal_reached_fn,
      const std::function<void(std::vector<SimpleRRTPlannerState<StateType, StateAllocator>>&, const int64_t)>&
          goal_reached_callback_fn,
      const std::function<SampleType(void)>& sampling_fn,
      const std::function<std::vector<std::pair<StateType, int64_t>>(const StateType&, const SampleType&)>&
      ...
      if (!propagated.empty()) {
        ...
        for (size_t idx = 0; idx < propagated.size(); idx++) {
          ...
          if (current_relative_parent_index >= 0) {
            ...
            node_parent_index = nearest_neighbor_index;  // Negative relative parent index means our parent index is the
    ...
    return std::pair<std::vector<std::vector<StateType, StateAllocator>>, std::map<std::string, double>>(planned_paths,
                                                                                                         statistics);
  }
  ...
  static std::vector<std::vector<T, Allocator>> ExtractSolutionPaths(
      const std::vector<SimpleRRTPlannerState<T, Allocator>>& nodes, const std::vector<int64_t>& goal_state_indices) {
   ...
   */
  template <typename T, typename Allocator = std::allocator<T>>
  static std::vector<T, Allocator> ExtractSolutionPath(const std::vector<SimpleRRTPlannerState<T, Allocator>>& nodes,
...
class SimpleHybridBiRRTPlanner {
 ...
 public:
  ...
  static std::pair<std::vector<T, Allocator>, std::map<std::string, double>> Plan(
      ...
      const std::function<std::vector<std::pair<T, int64_t>>(const T&, const T&)>& forward_propagation_fn,
      const double tree_sampling_bias, const double p_switch_tree, const std::chrono::duration<double>& time_limit,
      RNG& rng) {
    const std::function<void(std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t)> dummy_state_added_fn =
        [](std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t) { ; };
    const std::function<void(std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t,
                             std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t, const bool)>
        dummy_goal_bridge_callback_fn = [](std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t,
                                           std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t,
                                           const bool) { ; };
    const std::chrono::time_point<std::chrono::steady_clock> start_time = std::chrono::steady_clock::now();
    const std::function<bool(void)> termination_check_fn = [&](void) {
    ...
    bool solution_found = false;
    const std::function<bool(const T&, const T&)> real_states_connected_fn = [&](const T& state1, const T& state2) {
      if (states_connected_fn(state1, state2)) {
        solution_found = true;
        return true;
      } else {
        return false;
    ...
    const std::pair<std::vector<std::vector<T, Allocator>>, std::map<std::string, double>> planning_result =
        PlanMultiPath(start, goal, nearest_neighbor_fn, dummy_state_added_fn, real_states_connected_fn,
                      dummy_goal_bridge_callback_fn, state_sampling_fn, forward_propagation_fn, tree_sampling_bias,
                      p_switch_tree, real_termination_check_fn, rng);
    // Put together the return
    std::vector<T, Allocator> planned_path;
    if (planning_result.first.size() > 0) {
      planned_path = planning_result.first[0];
  ...
  static std::pair<std::vector<std::vector<T, Allocator>>, std::map<std::string, double>> PlanMultiPath(
      ...
      const std::function<T(void)>& state_sampling_fn,
      const std::function<std::vector<std::pair<T, int64_t>>(const T&, const T&)>& forward_propagation_fn,
      const double tree_sampling_bias, const double p_switch_tree,
      const std::function<bool(void)>& termination_check_fn, RNG& rng) {
    const std::function<void(std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t)> dummy_state_added_fn =
        [](std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t) { ; };
    const std::function<void(std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t,
                             std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t, const bool)>
        dummy_goal_bridge_callback_fn = [](std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t,
                                           std::vector<SimpleRRTPlannerState<T, Allocator>>&, const int64_t,
                                           const bool) { ; };
    return PlanMultiPath(start, goal, nearest_neighbor_fn, dummy_state_added_fn, states_connected_fn,
                         dummy_goal_bridge_callback_fn, state_sampling_fn, forward_propagation_fn, tree_sampling_bias,
                         p_switch_tree, termination_check_fn, rng);
  }
   ...
   * goal - target configuration
   * nearest_neighbor_fn - given all nodes explored so far, and a new state, return the index of the "closest" node
   * states_connected_fn - return if a the given states meet conditions to connect the trees (for example, within a
   * radius) state_sampling_fn - returns a new state (randomly- or deterministically-sampled) forward_propagation_fn -
   * given the nearest neighbor and a new target state, returns the states that would grow the tree towards the target
  ...
  static std::pair<std::vector<std::vector<T>>, std::map<std::string, double>> PlanMultiPath(
    ...
    return PlanMultiPath(start_tree, goal_tree, nearest_neighbor_fn, state_added_fn, states_connected_fn,
                         goal_bridge_callback_fn, state_sampling_fn, forward_propagation_fn, tree_sampling_bias,
                         p_switch_tree, termination_check_fn, rng);
  }
  ...
  static std::pair<std::vector<std::vector<StateType, Allocator>>, std::map<std::string, double>> PlanMultiPath(
      ...
      const std::function<int64_t(const std::vector<SimpleRRTPlannerState<StateType, Allocator>>&, const StateType&)>&
          nearest_neighbor_fn,
      const std::function<void(std::vector<SimpleRRTPlannerState<StateType, Allocator>>&, const int64_t)>&
          state_added_fn,
      const std::function<bool(const StateType&, const StateType&)>& states_connected_fn,
      const std::function<void(std::vector<SimpleRRTPlannerState<StateType, Allocator>>&, const int64_t,
                               std::vector<SimpleRRTPlannerState<StateType, Allocator>>&, const int64_t, const bool)>&
          goal_bridge_callback_fn,
      const std::function<StateType(void)>& state_sampling_fn,
      const std::function<std::vector<std::pair<StateType, int64_t>>(const StateType&, const StateType&)>&
          forward_propagation_fn,
      const double tree_sampling_bias, const double p_switch_tree,
      const std::function<bool(void)>& termination_check_fn, RNG& rng) {
    if (start_tree.empty()) {
    ...
    for (size_t start_tree_idx = 0; start_tree_idx < start_tree.size(); start_tree_idx++) {
      for (size_t goal_tree_idx = 0; goal_tree_idx < goal_tree.size(); goal_tree_idx++) {
        if (states_connected_fn(start_tree[start_tree_idx].GetValueImmutable(),
                                goal_tree[goal_tree_idx].GetValueImmutable())) {
          std::cerr << "Starting pair (" << start_tree_idx << ", " << goal_tree_idx
    ...
    while (!termination_check_fn()) {
      ...
      std::vector<SimpleRRTPlannerState<StateType, Allocator>>& active_tree =
          (start_tree_active) ? start_tree : goal_tree;
      std::vector<SimpleRRTPlannerState<StateType, Allocator>>& target_tree =
      ...
      const StateType target_state =
          (sample_from_tree) ? target_tree.at(target_tree_node_index).GetValueImmutable() : state_sampling_fn();
      // Get the nearest neighbor
    ...
    for (size_t goal_bridge_idx = 0; goal_bridge_idx < goal_bridges.size(); goal_bridge_idx++) {
      ...
      std::vector<StateType, Allocator> start_path =
          SimpleHybridRRTPlanner::ExtractSolutionPath(start_tree, goal_bridge.first);
      // Extract the portion in the goal tree
      std::vector<StateType, Allocator> goal_path =
          SimpleHybridRRTPlanner::ExtractSolutionPath(goal_tree, goal_bridge.second);
    ...
    statistics["solutions"] = (double)planned_paths.size();
    return std::pair<std::vector<std::vector<StateType, Allocator>>, std::map<std::string, double>>(planned_paths,
                                                                                                    statistics);
  }
};
