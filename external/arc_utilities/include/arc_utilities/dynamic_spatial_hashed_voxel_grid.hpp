...
class DynamicSpatialHashedVoxelGridChunk {
 protected:
  ...
  inline int64_t GetLocationDataIndex(const Eigen::Vector3d& location) const {
    ...
    if (location_in_chunk.x() < 0.0 || location_in_chunk.y() < 0.0 || location_in_chunk.z() < 0.0) {
      return -1;
    } else if (location_in_chunk.x() > chunk_x_size_ || location_in_chunk.y() > chunk_y_size_ ||
               location_in_chunk.z() > chunk_z_size_) {
      return -1;
    }
    // Ok, we're inside the chunk
      ...
      if (x_cell < 0 || y_cell < 0 || z_cell < 0 || x_cell >= num_x_cells_ || y_cell >= num_y_cells_ ||
          z_cell >= num_z_cells_) {
        return -1;
      } else {
 ...
 public:
  ...
  DynamicSpatialHashedVoxelGridChunk(const CHUNK_REGION& region, const double cell_x_size, const double cell_y_size,
                                     const double cell_z_size, const int64_t num_x_cells, const int64_t num_y_cells,
                                     const int64_t num_z_cells, const T& initial_value) {
  ...
  DynamicSpatialHashedVoxelGridChunk(const CHUNK_REGION& region, const double chunk_x_size, const double chunk_y_size,
                                     const double chunk_z_size, const T& initial_value) {
  ...
  bool IsCellInitialized() const { return cell_initialized_; }

  bool IsChunkInitialized() const { return chunk_initialized_; }
  ...
  inline std::pair<const T&, bool> GetImmutableByIndex(const int64_t x_index, const int64_t y_index,
  ...
  inline std::vector<double> GetIndexLocationInGrid(const int64_t x_index, const int64_t y_index,
                                                    const int64_t z_index) const {
    assert(chunk_initialized_ || cell_initialized_);
    if (IndexInBounds(x_index, y_index, z_index)) {
      if (chunk_initialized_) {
        ...
        Eigen::Vector3d point_in_chunk_frame(cell_x_size_ * ((double)x_index + 0.5),
...
class DynamicSpatialHashedVoxelGrid {
 protected:
  ...
  inline void SafetyCheckSizes(const double cell_x_size, const double cell_y_size, const double cell_z_size,
                               const int64_t chunk_num_x_cells, const int64_t chunk_num_y_cells,
                               const int64_t chunk_num_z_cells) const {
    if (cell_x_size <= 0.0) {
      throw std::invalid_argument("cell_x_size must be positive and non-zero");
    }
    if (std::isnan(cell_x_size)) {
  ...
  inline void CoreInitialize(const double cell_x_size, const double cell_y_size, const double cell_z_size,
                             const int64_t chunk_num_x_cells, const int64_t chunk_num_y_cells,
                             const int64_t chunk_num_z_cells, const T& default_value) {
 ...
 public:
  ...
  DynamicSpatialHashedVoxelGrid(const Eigen::Isometry3d& origin_transform, const double cell_x_size,
                                const double cell_y_size, const double cell_z_size, const int64_t chunk_num_x_cells,
                                const int64_t chunk_num_y_cells, const int64_t chunk_num_z_cells,
                                const T& default_value) {
    Initialize(origin_transform, cell_x_size, cell_y_size, cell_z_size, chunk_num_x_cells, chunk_num_y_cells,
               chunk_num_z_cells, default_value);
  }

  DynamicSpatialHashedVoxelGrid(const Eigen::Isometry3d& origin_transform, const double cell_size,
                                const int64_t chunk_num_x_cells, const int64_t chunk_num_y_cells,
                                const int64_t chunk_num_z_cells, const T& default_value) {
    Initialize(origin_transform, cell_size, cell_size, cell_size, chunk_num_x_cells, chunk_num_y_cells,
               chunk_num_z_cells, default_value);
  }

  DynamicSpatialHashedVoxelGrid(const double cell_x_size, const double cell_y_size, const double cell_z_size,
                                const int64_t chunk_num_x_cells, const int64_t chunk_num_y_cells,
                                const int64_t chunk_num_z_cells, const T& default_value) {
    Initialize(cell_x_size, cell_y_size, cell_z_size, chunk_num_x_cells, chunk_num_y_cells, chunk_num_z_cells,
               default_value);
  }

  DynamicSpatialHashedVoxelGrid(const double cell_size, const int64_t chunk_num_x_cells,
                                const int64_t chunk_num_y_cells, const int64_t chunk_num_z_cells,
                                const T& default_value) {
  ...
  inline void Initialize(const Eigen::Isometry3d& origin_transform, const double cell_x_size, const double cell_y_size,
                         const double cell_z_size, const int64_t chunk_num_x_cells, const int64_t chunk_num_y_cells,
                         const int64_t chunk_num_z_cells, const T& default_value) {
    SafetyCheckSizes(cell_x_size, cell_y_size, cell_z_size, chunk_num_x_cells, chunk_num_y_cells, chunk_num_z_cells);
    CoreInitialize(cell_x_size, cell_y_size, cell_z_size, chunk_num_x_cells, chunk_num_y_cells, chunk_num_z_cells,
                   default_value);
  ...
  inline SET_STATUS SetCellValue(const Eigen::Vector3d& location, const T& value) {
    ...
    if (found_chunk_itr != chunks_.end()) {
      ...
      if (chunk.IsCellInitialized()) {
        ...
        // Make a new chunk
        DynamicSpatialHashedVoxelGridChunk<T, Allocator> new_chunk(region, cell_x_size_, cell_y_size_, cell_z_size_,
                                                                   chunk_num_x_cells_, chunk_num_y_cells_,
                                                                   chunk_num_z_cells_, current_chunk_value);
        ...
        DynamicSpatialHashedVoxelGridChunk<T, Allocator> new_chunk(region, cell_x_size_, cell_y_size_, cell_z_size_,
                                                                   chunk_num_x_cells_, chunk_num_y_cells_,
                                                                   chunk_num_z_cells_, default_value_);
        if (new_chunk.SetCellValue(grid_location, value)) {
          chunks_[region] = new_chunk;
          return SET_CELL;
        } else {
  ...
  inline SET_STATUS SetChunkValue(const Eigen::Vector3d& location, const T& value) {
    ...
    if (found_chunk_itr != chunks_.end()) {
      ...
      if (chunk.IsCellInitialized()) {
        // Make a new chunk
        DynamicSpatialHashedVoxelGridChunk<T, Allocator> new_chunk(region, chunk_x_size_, chunk_y_size_, chunk_z_size_,
                                                                   value);
        chunks_[region] = new_chunk;
        return SET_CHUNK;
      } else if (chunk.IsChunkInitialized()) {
  ...
  inline std::vector<double> GetChunkSizes() const {
    return std::vector<double>{cell_x_size_ * (double)chunk_num_x_cells_, cell_y_size_ * (double)chunk_num_y_cells_,
  ...
  inline Eigen::Isometry3d GetOriginTransform() const { return origin_transform_; }
